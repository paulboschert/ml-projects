...
class Knearest:
    ...
    def classify(self, example):
        ...
        # use BallTree's query to get the distances and indices of the k-nearest neighbors
        distances, indices = self._kdtree.query(example, k=self._k)

        # since query() returns a list within a list, we must send in the inner list to majority()
    ...
    def confusion_matrix(self, test_x, test_y):
        ...
        for xx, yy in zip(test_x, test_y):
            ...
            # classify xx! aka guess...
            guess = self.classify(xx);

            # increment the number of times answer yy has been classified as 'guess'
            try:
                d[yy][guess] += 1;
            except KeyError: # if there does not yet exist an entry in the dictionary, create one
                d[yy][guess] = 1;
...
if __name__ == "__main__":
    ...
    dataSizeStart = kStart;
    dataSizeIncrement = 100;
    dataSizeMaximum = 1900; #len(data.train_x);

    k = kStart 
    print("\n\nk\tn\taccuracy")
    while k <= kMaximum:
        n = max(dataSizeStart, k);
        while n <= dataSizeMaximum:
            knn = Knearest(data.train_x[:n], data.train_y[:n], k)
            confusion = knn.confusion_matrix(data.test_x, data.test_y)
            accuracy = knn.acccuracy(confusion)

            print("%d\t%d\t%f" % (k, n, accuracy))
            if n % dataSizeIncrement != 0:
                n = dataSizeIncrement
            else:
                n += dataSizeIncrement
        k += kIncrement
